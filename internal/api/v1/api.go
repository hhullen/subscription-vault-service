...
//go:generate mockgen -source=api.go -destination=api_mock.go -package=api ISubscriptionService,IServer,IRouter,ILogger
...
func NewAPI(ctx context.Context, address string,
	...
	return buildAPI(ctx, subs, log, server, router), nil
}
